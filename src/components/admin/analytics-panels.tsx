...
import { categoryDistribution, resolutionTrend } from "@/lib/admin-data";
  ...
  return (
    <div className="h-60 px-2 pb-3 pt-4">
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={resolutionTrend} margin={{ top: 8, right: 14, left: -18, bottom: 0 }}>
          <defs>
  ...
  return (
    <div className="grid min-h-60 grid-cols-[45%_55%] items-center px-4 py-3">
      <div className="h-44">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={categoryDistribution} dataKey="value" innerRadius="58%" outerRadius="82%" paddingAngle={3} stroke="none">
              {categoryDistribution.map((item) => <Cell key={item.name} fill={item.color} />)}
            </Pie>
